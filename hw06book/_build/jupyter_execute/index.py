...
template_p_whiten = ut.whiten(template_p,psd_H1,dt)
template_c_whiten = ut.whiten(template_c,psd_H1,dt)
...
template_p_smooth = ut.whiten(template_p,psd_smooth,dt)
...
ut.write_wavfile(eventname+"_template_whiten.wav",int(fs), template_p_smooth[indxt])
...
strain_L1_shifted = ut.reqshift(strain_L1_whitenbp,fshift=fshift,sample_rate=fs)

# write the files:
ut.write_wavfile(eventname+"_H1_shifted.wav",int(fs), strain_H1_shifted[indxd])
ut.write_wavfile(eventname+"_L1_shifted.wav",int(fs), strain_L1_shifted[indxd])

# and the template:
template_p_shifted = ut.reqshift(template_p_smooth,fshift=fshift,sample_rate=fs)
ut.write_wavfile(eventname+"_template_shifted.wav",int(fs), template_p_shifted[indxt])
